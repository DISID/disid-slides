...
aceleran el proceso de desarrollo debido a que especifica y define partes 
de los procesos propios del ciclo de vida de los proyectos de desarrollo de 
software:
...
[role="incremental scatter"]
* *Múltiples* frameworks para una aplicación
* Frameworks solucionan *aspectos concretos*
...
No sólo eso dependiendo del tipo de proyecto se puede perder la homogeneidad
porque la plataforma destino ofrece unas herramientas de desarrollo
específicas que son las que se suelen utilizar porque da sensación de rapidez
o porque efectivamente desarrollar siguiendo los estándares marcados es más
complicado que utilizar a las herramientas del proveedor.
...
utiliza que debe ser única para cualquier tipo de proyecto.
...
beneficios de un herramienta de desarrollo de aplicaciones web dinámicas.
